    ...
    '<div class="list-group">' +
      '{{#each view.moduleObj.moduleData}}' +
        '<a class="list-group-item module-data">' +
          ...
          '{{#if hasWinners}}' +
            '<div class="list-group-item-text">First ({{challengeDataObj.first}}) : {{firstName}} {{#if canAddFirst}}<span class="btn btn-link btn-placement" {{action "addSelfToPos" this view.moduleObj "firstId"}}>{{#tool-tip title="Add Self"}}<span class="glyphicon glyphicon-plus"></span>{{/tool-tip}}</span>{{/if}}{{#if placedFirst}}<span class="btn btn-link btn-placement" {{action "removeSelfFromPos" this view.moduleObj "firstId"}}>{{#tool-tip title="Remove Self"}}<span class="glyphicon glyphicon-trash"></span>{{/tool-tip}}</span>{{/if}}</div>' +
            '<div class="list-group-item-text">Second ({{challengeDataObj.second}}) : {{secondName}} {{#if canAddSecond}}<span class="btn btn-link btn-placement" {{action "addSelfToPos" this view.moduleObj "secondId"}}>{{#tool-tip title="Add Self"}}<span class="glyphicon glyphicon-plus"></span>{{/tool-tip}}</span>{{/if}}{{#if placedSecond}}<span class="btn btn-link btn-placement" {{action "removeSelfFromPos" this view.moduleObj "secondId"}}>{{#tool-tip title="Remove Self"}}<span class="glyphicon glyphicon-trash"></span>{{/tool-tip}}</span>{{/if}}</div>' +
            '<div class="list-group-item-text">Third ({{challengeDataObj.third}}) : {{thirdName}} {{#if canAddThird}}<span class="btn btn-link btn-placement" {{action "addSelfToPos" this view.moduleObj "thirdId"}}>{{#tool-tip title="Add Self"}}<span class="glyphicon glyphicon-plus"></span>{{/tool-tip}}</span>{{/if}}{{#if placedThird}}<span class="btn btn-link btn-placement" {{action "removeSelfFromPos" this view.moduleObj "thirdId"}}>{{#tool-tip title="Remove Self"}}<span class="glyphicon glyphicon-trash"></span>{{/tool-tip}}</span>{{/if}}</div>' +
          '{{/if}}' +
        '</a>' +
      '{{else}}' +
        'Empty!' +
